...
export const register = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ msg: "success", data: sanitizeUser(user), userToken: token });
});
...
export const login = asyncHandler(async (req, res, next) => {
  ...
  const token = Jwt.sign(
    ...
    process.env.JWT_SECRET_KEY,
    { expiresIn: rememberMe ? process.env.JWT_EXPIRE_DATE : "15d" }
  );


  // Store in Cookie for Persistence

  res.status(200).json({ message: "success", data: sanitizeUser(user), userToken: token });
});
